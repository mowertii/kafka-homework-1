...
**Зачем нужен retry?**
Чтобы при временных сбоях (сеть, БД) сообщение не терялось, а обрабатывалось повторно.

**Зачем нужен backoff?**
Чтобы не "ддосить" падающую систему — дать ей время восстановиться.

**Для чего используется DLT?**
Для сообщений, которые не удалось обработать после всех попыток — их нужно разбирать вручную.

**Что делать с сообщениями в DLT?**
*Мониторить размер DLT (alert)
*Анализировать причину ошибки
*Исправлять баг в коде
*Переотправлять сообщения обратно (replay)
*Удалять безнадежные
